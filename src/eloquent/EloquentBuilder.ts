...
export class EloquentBuilder<T extends Model> {
  ...
  with(relations: string | string[] | Record<string, any>): this {
    ...
    const registerNestedPairs = (segments: string[]) => {
      for (let i = 0; i < segments.length - 1; i++) {
        ...
        if (!this.nestedRelations.has(parent))
          this.nestedRelations.set(parent, new Set());
  ...
  async paginate(
    ...
    const pp = Number.isFinite(Number(perPage))
      ? Math.max(1, Math.floor(Number(perPage)))
      : 15;
    const pg = Number.isFinite(Number(page))
  ...
  async increment(column: string, amount: number = 1): Promise<number> {
    if (getDbType() === "mongodb")
      return this.updateMongo({ [column]: { $inc: amount } } as any);
    const tableName = (this.model as typeof Model).getTable();
    const where = this.buildWhereClause();
  ...
  async decrement(column: string, amount: number = 1): Promise<number> {
    if (getDbType() === "mongodb")
      return this.updateMongo({ [column]: { $dec: amount } } as any); // treat as custom
    const tableName = (this.model as typeof Model).getTable();
    const where = this.buildWhereClause();
    const sql = `UPDATE ${tableName} SET ${column} = ${column} - ?${where.sql}`;
    const params = [amount, ...where.params];
  ...
  private async aggregate(
    ...
    if (this.havingClauses.length > 0) {
      const havingParts = this.havingClauses.map(
  ...
  private buildNestedWhere(clauses: WhereClause[]): {
    sql: string;
    params: any[];
  } {
    const parts: string[] = [];
    ...
    clauses.forEach((w, idx) => {
      ...
      if (Array.isArray(w.value) && (w.operator || "").toLowerCase() === "in") {
        ...
        parts.push(
          `${boolOp}${qualify(w.column)} ${(w.operator || "=").toUpperCase()} ?`,
        );
        params.push(w.value);
      }
    });
  ...
  private buildHasConditionsSQL(
    ...
    builder.hasConditions.forEach((cond) => {
      ...
      const softDeleteFragment = (relatedModel as any).softDeletes
    ...
    return {
      sql: clauses
        .map((c, i) => `${clauseBooleans[i].toUpperCase()} ${c}`)
        .join(" "),
      params,
    };
  }

  private async applyHasConditionsMongo(
    docs: any[],
    originalWhereFilter?: any,
  ): Promise<any[]> {
    ...
    for (const doc of docs) {
      ...
      for (const cond of this.hasConditions) {
        ...
        const applyNestedHasForRelatedDocs = async (
          ...
          if (!cb || !cb.hasConditions || cb.hasConditions.length === 0)
            return docsArr.length;
          ...
          for (const rd of docsArr) {
            ...
            for (const nestedCond of cb.hasConditions) {
              ...
              const relMeta2 = (
                new (currentLevelModel as any)() as any
              ).getRelationship(nestedCond.relation);
              if (!relMeta2) {
                okAll = false;
                break;
              }
              ...
              if (
                ...
                const nestedBuilder = nestedCond.callback
                  ? new EloquentBuilder(nestedModel)
                  : null;
                if (nestedBuilder && nestedCond.callback)
                  nestedCond.callback(nestedBuilder as any);
                const baseFilter2: any = { [fk]: rd[currentLevelPK] };
                if ((nestedModel as any).softDeletes)
                  baseFilter2.deleted_at = null;
                if (nestedBuilder) {
                ...
                if (fkVal2 === undefined || fkVal2 === null) innerActual = 0;
                  ...
                  const normalizedOwnerKey2 =
                    ownerKey2 === "id" ? "_id" : ownerKey2;
                  const coercedFkVal2 =
                    normalizedOwnerKey2 === "_id"
                      ? this.coerceId(fkVal2)
                      : fkVal2;
                  const baseFilter2: any = {
                    [normalizedOwnerKey2]: coercedFkVal2,
                  };
                  if ((nestedModel as any).softDeletes)
                    baseFilter2.deleted_at = null;
                  if (nestedCond.callback) {
                    ...
                    if (
                      ...
                      const nestedDocs = await c2
                        .find(filters2 as any)
                        .toArray();
                      innerActual = await applyNestedHasForRelatedDocs(
                        nestedDocs,
                        nestedBuilder,
                ...
                const foreignPivotKey2 =
                  relMeta2.foreignKey || `${currentLevelTable}_id`;
                const relatedPivotKey2 =
                  relMeta2.relatedKey || `${nestedTable}_id`;
                ...
                if (!pivots2.length) innerActual = 0;
                  ...
                  const relatedIds2 = Array.from(
                    new Set(pivots2.map((p) => p[relatedPivotKey2])),
                  );
                  if (!relatedIds2.length) innerActual = 0;
                    ...
                    if ((nestedModel as any).softDeletes)
                      filter2.deleted_at = null;
                    if (nestedCond.callback) {
                      const nestedBuilder = new EloquentBuilder(nestedModel);
                      nestedCond.callback(nestedBuilder as any);
                      const extra2 = (nestedBuilder as any).buildMongoFilter();
                      if (Object.keys(extra2).length)
                        filter2 = { $and: [filter2, extra2] };
                      // Recursively apply nested has conditions
                      if (
                        (nestedBuilder as any).hasConditions &&
                        (nestedBuilder as any).hasConditions.length
                      ) {
                        const nestedDocs = await rc2
                          .find(filter2 as any)
                          .toArray();
                        innerActual = await applyNestedHasForRelatedDocs(
                          nestedDocs,
        ...
        if (
          ...
          const foreignKey =
            relMeta.foreignKey || `${(this.model as any).getTable()}_id`;
          const c = mongoCollection(relatedTable);
          const constraintBuilder = cond.callback
            ? new EloquentBuilder(relatedModel)
            : null;
          if (constraintBuilder && cond.callback)
          ...
          if (constraintBuilder) {
            ...
            const filters = Object.keys(extra).length
              ...
              : baseFilter;
            if (
              constraintBuilder.hasConditions &&
              constraintBuilder.hasConditions.length
            ) {
              const arr = await c.find(filters as any).toArray();
              actual = await applyNestedHasForRelatedDocs(
                arr,
                constraintBuilder as any,
              );
            } else {
          ...
          if (fkVal === undefined || fkVal === null) actual = 0;
            ...
            const coercedFkVal =
              normalizedOwnerKey === "_id" ? this.coerceId(fkVal) : fkVal;
          ...
          const pivot = relMeta.table;
          const foreignPivotKey =
            relMeta.foreignKey || `${(this.model as any).getTable()}_id`;
          const relatedPivotKey = relMeta.relatedKey || `${relatedTable}_id`;
          const pc = mongoCollection(pivot);
          const pivots = await pc
            .find({ [foreignPivotKey]: doc[parentPK] })
            .toArray();
          if (!pivots.length) actual = 0;
          else {
            const relatedIds = Array.from(
              new Set(pivots.map((p) => p[relatedPivotKey])),
            );
  ...
  private async executeQuery(): Promise<any[]> {
    ...
    const columns =
      this.selectedColumns && this.selectedColumns.length
    ...
    if (this.orderByColumn) {
      parts.push(
        `ORDER BY ${this.orderByColumn} ${this.orderByDirection.toUpperCase()}`,
      );
    }
    ...
    for (const relName of Object.keys(tree)) {
      ...
      if (parentPath === "") {
        ...
        const sampleModel = currentModels.find(
          (m) =>
            typeof m?.getRelationship === "function" &&
            m.getRelationship(relName),
        );
        if (!sampleModel) continue;
        const tmpBuilder = new EloquentBuilder(
          sampleModel.constructor as typeof Model,
        );
        await tmpBuilder.loadRelationship(
          currentModels as any,
          relName,
          options,
        );
      }
  ...
  private setRelation(model: T, name: string, value: any) {
    (model as any).relationshipsLoaded =
      (model as any).relationshipsLoaded || {};
  ...
  private async loadHasOne(
    ...
    const foreignKey =
      rel.foreignKey || `${(this.model as typeof Model).getTable()}_id`;
  ...
  private async loadBelongsToMany(
    ...
    const parentPK = (this.model as any).primaryKey || "id";
    const foreignPivotKey =
      rel.foreignKey || `${(this.model as any).getTable()}_id`;
    ...
    const relatedIds = Array.from(
      new Set(pivotRows.map((r: any) => r.related_id)),
    );
  ...
  private async loadBelongsToManyMongo(
    ...
    const pivots = await pc
      .find({ [foreignPivotKey]: { $in: parentMatch } })
      .toArray();
    if (!pivots.length) {
      models.forEach((m) => this.setRelation(m, relation, []));
      return;
    ...
    const relatedTokens = Array.from(
      new Set(pivots.map((p: any) => String(p[relatedPivotKey]))),
    );
    const rc = mongoCollection(relatedTable);
    let rows: any[] = [];
    if (relatedPK === "id") {
      ...
      if (shortTokens.length) {
        ...
        const prefixExprs = tokenSet.map((t) => ({
          $expr: {
            $eq: [{ $substrBytes: [{ $toString: "$_id" }, 0, t.length] }, t],
          },
        }));
        or.push(...prefixExprs);
      }
      ...
      rows = await rc
        .find({ [relatedPK]: { $in: relatedTokens } as any })
        .toArray();
    }
  ...
  private async loadMorphRelations(
    ...
    if (!localIds.length) {
      models.forEach((m) =>
        this.setRelation(m, relation, rel.type === "morphOne" ? null : []),
      );
      return;
    }
    ...
    if (rel.type === "morphOne") {
      ...
      models.forEach((m) => {
        ...
        const uniq = Array.from(
          ...
          acc.find((x) => (x._id ? String(x._id) : JSON.stringify(x)) === id),
        );
  ...
  private docMatchesMongoFilter(doc: any, filter: any): boolean {
    ...
    for (const [key, condition] of Object.entries(filter)) {
      if (key === "$and") {
        if (
          !(condition as any[]).every((f: any) =>
            this.docMatchesMongoFilter(doc, f),
          )
        )
          return false;
        continue;
        ...
        for (const [op, val] of Object.entries(
          ...
          switch (op) {
            case "$in":
              if (
                !Array.isArray(val) ||
                !val.some((v: any) => String(v) === String(docValue))
              )
                return false;
              break;
            case "$nin":
            ...
            case "$regex": {
              ...
              if (!new RegExp(val as string, flags).test(String(docValue)))
                return false;
              break;
            }
            default:
  ...
  private buildMongoFilter(): any {
    ...
    const toFilter = (w: WhereClause): any => {
      ...
      switch (op) {
        ...
        case "like":
          return {
            [col]: { $regex: String(v).replace(/%/g, ".*"), $options: "i" },
          };
        default:
          return { [col]: v };
      }
  ...
  private async executeQueryMongo(): Promise<any[]> {
    ...
    const proj =
      ...
      this.selectedColumns[0] !== "*"
        ? Object.fromEntries(
            this.selectedColumns.map((k) => [this.normalizeField(k), 1]),
          )
        : undefined;
    ...
    const hasOrConditions = this.hasConditions.some(
      (cond) => (cond.boolean || "and") === "or",
    );
      ...
      for (const cond of this.hasConditions) {
        ...
        if (relMeta.type === "belongsTo") {
          ...
          if (matchingIds.length) {
            orBranches.push({
              [this.normalizeField(foreignKey)]: { $in: matchingIds },
            });
          }
          ...
          const matchingDocs = await rc
            ...
            .toArray();
          const parentIds = matchingDocs
            .map((d: any) => d[foreignKey])
            .filter(Boolean);
          if (parentIds.length) {
            orBranches.push({
              [this.normalizeField(parentPK)]: { $in: parentIds },
            });
          }
        }
    ...
    if (this.orderByColumn) {
      cursor = cursor.sort({
        [this.normalizeField(this.orderByColumn)]:
          this.orderByDirection === "asc" ? 1 : -1,
      });
    }
    ...
    if (
      Object.values(values).some(
        (v) =>
          v &&
          typeof v === "object" &&
          ("$inc" in (v as any) || "$dec" in (v as any)),
      )
    ) {
      ...
      for (const [k, v] of Object.entries(values)) {
        if (v && typeof v === "object" && (v as any).$inc !== undefined) {
          ...
          v &&
          typeof v === "object" &&
          (v as any).$dec !== undefined
        ) {
          updateDoc.$inc = updateDoc.$inc || {};
          (updateDoc.$inc as any)[this.normalizeField(k)] = -(v as any).$dec;
        } else {
          updateDoc.$set = updateDoc.$set || {};
          (updateDoc.$set as any)[this.normalizeField(k)] = this.coerceForField(
            k,
            v as any,
          );
        }
  ...
  private async aggregateMongo(fn: string, column: string): Promise<string> {
    ...
    if (this.hasConditions.length) {
      ...
      const kept = await this.applyHasConditionsMongo(
        docs,
        originalWhereFilter,
      );
      ...
      if (this.distinctValue && column !== "*") {
        ...
        values = Array.from(uniqueVals).map((v) =>
      ...
      if (key === "count") {
        return String(
          this.distinctValue && column !== "*" ? values.length : kept.length,
        );
      }
    ...
    if (this.groupByColumns.length > 0) {
      ...
      pipeline.push({
        $group: { _id: groupId, agg: aggOp[key] || { $sum: 1 } },
      });
      ...
      if (this.havingClauses.length > 0) {
        ...
        this.havingClauses.forEach((h) => {
          const col =
            h.column === "agg" ? "agg" : this.normalizeField(h.column);
  ...
  private async loadHasOneMongo(
    ...
    if (localKey === "id") {
      matchVals = Array.from(
        new Set(localsRaw.flatMap((v) => this.expandIdForms(v))),
      );
    }
    const c = mongoCollection(relatedTable);
  ...
  private async loadMorphRelationsMongo(
    ...
    const matchVals = Array.from(
      new Set(localsRaw.flatMap((v) => this.expandIdForms(v))),
    );
    const c = mongoCollection(relatedTable);
    const rows = await c
      .find({
        [foreignKey]: { $in: matchVals },
        [morphTypeKey]: morphType,
      } as any)
      .toArray();
    if (rel.type === "morphOne") {
      const byFK = new Map<any, any>();
      ...
      models.forEach((m) => {
        ...
        const row =
          cand.map((v) => byFK.get(v) || byFK.get(String(v))).find(Boolean) ||
          null;
        if (row) {
          const inst = new (relatedModel as any)();
          inst.hydrate(row);
