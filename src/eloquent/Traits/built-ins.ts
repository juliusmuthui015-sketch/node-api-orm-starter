...
export class Sluggable {
    ...
    static boot(modelClass: typeof Model): void {
        ...
        modelClass.on('saving', (model: Model) => {
...
export class Sortable {
    ...
    async moveUp(): Promise<void> {
        const model = this as any as Model;
        const m: any = model as any;
        const currentOrder: number = typeof m.getAttribute === 'function' ? m.getAttribute('order') : m.order;
...
export class Searchable {
     ...
     */
    @scopeMethod()
    static scopeSearch(builder: EloquentBuilder<any>, query: string, fields: string[] = ['name', 'description']) {
        if (query) {
