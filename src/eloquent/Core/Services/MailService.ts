    ...
    private createDriver(name: string): MailDriverInterface {
        ...
        switch (config.transport) {
            ...
            case 'postmark':
                ...
                console.warn(`[Mail] Driver ${config.transport} not fully implemented, falling back to log`);
...
export class Mailer implements MailerInterface {
     ...
     */
    private normalizeAddresses(addresses: MailAddress | MailAddress[] | string | string[]): MailAddress[] {
...
export const MailService = {
    ...
    later: (mailable: MailableInterface, delay: number, queue?: string) => Mail().later(mailable, delay, queue),
};
