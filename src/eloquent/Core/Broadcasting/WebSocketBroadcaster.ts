...
export class WebSocketBroadcaster implements BroadcasterDriver {
     ...
     */
    private async handleSubscribe(connection: BroadcastConnection, channelName: string): Promise<void> {
    ...
    async broadcast(message: BroadcastMessage): Promise<void> {
        ...
        for (const channelName of message.channels) {
            this.broadcastToChannel(channelName, { ...serverMessage, channel: channelName }, message.socket);
        }
    }
