...
export class TableBuilder {
  ...
  string(name: string, length = 191) {
    return this.column('VARCHAR', name, length);
  }
  decimal(name: string, length: number = 10, decimalPlaces: number = 2) {
    return this.column('DECIMAL', name, length);
  }
  char(name: string, length = 191) {
  ...
  json(name: string, _length?: number) {
    return this.column('JSON', name);
  }
  datetime(name: string) {
    return this.column('DATETIME', name);
  }
  timestamp(name: string) {
    const c = this.column('TIMESTAMP', name);
    c.nullable();
    return c;
  }
