...
export async function seed() {
  ...
  let existingUserRole = await Role.where('slug', 'user').first();
  if (!existingUserRole) {
    await Role.create({
      name: 'User',
      slug: 'user',
    ...
    existingUserRole = await Role.where('slug', 'landlord').first();
    if (!existingUserRole) {
        await Role.create({
            name: 'Landlord',
            slug: 'landlord',
    ...
    existingUserRole = await Role.where('slug', 'tenant').first();
    if (!existingUserRole) {
        await Role.create({
            name: 'Tenant',
            slug: 'tenant',
  ...
  if (!existingUser) {
    ...
    await existingUser.profile().create({
      user_id: existingUser.id as any,
      gender: 'male',
