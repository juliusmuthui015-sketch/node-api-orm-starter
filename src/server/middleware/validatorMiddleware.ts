...
declare global {
    namespace Express { interface Request { validate: (payloadOrRules?: any, rulesMaybe?: Record<string, RuleSpec> | Record<string, string | RuleFn>, customMessages?: Record<string,string>) => Promise<any>  } }
}
...
export default function validatorMiddleware(req: Request & { validate?: any }, res: Response, next: NextFunction) {
  ...
  req.validate = async function(payloadOrRules: any, maybeRules?: any, customMessages?: Record<string,string>) {
    let payload = undefined as any;
    let rules = undefined as any;
    if (maybeRules === undefined && typeof payloadOrRules === 'object' && !Array.isArray(payloadOrRules) && Object.keys(payloadOrRules || {}).length && Object.values(payloadOrRules).every(v => typeof v === 'string' || typeof v === 'function' || (typeof v === 'object' && v && 'rule' in v))) {
      ...
      throw new Error('Invalid validate invocation. Use req.validate(payload, rules, customMessages) or req.validate(rules, customMessages)');
