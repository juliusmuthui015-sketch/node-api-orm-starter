        ...
        for (const p of parts) {
            ...
            switch (true) {
                ...
                case p === 'confirmed':
                    ...
                    if (payload && payload[confirmationField] !== val) {
...
export const requiredIf = (otherField: string, value: any): RuleFn =>
...
export const requiredUnless = (otherField: string, value: any): RuleFn =>
    async (val, field, payload) => {
        if (payload && payload[otherField] !== value) {
            if (val === undefined || val === null || val === '') {
                return { ok: false, message: 'required' };
...
export const mimes = (allowedTypes: string[]): RuleFn =>
    async (value) => {
        if (!value) return true;
...
export const maxFileSize = (maxSizeInMB: number): RuleFn =>
    async (value) => {
        if (!value) return true;
...
export const nestedRule = (rules: Record<string, RuleSpec>): RuleFn =>
...
export const arrayOfObjectsRule = (rules: Record<string, RuleSpec>): RuleFn =>
    async (value, field, payload) => {
        if (value === undefined || value === null) return true;
